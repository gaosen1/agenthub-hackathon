...
def find_pids(agent):
    """识别优先级：state/<name>.pid > session_log(lsof) > pgrep -f match"""
    ...
    if os.path.isfile(pid_file):
        try:
            ...
            return []
    # 交互式会话：命令行无 session-id，用其持续打开的 segments 日志文件识别
    session_log = agent.get("session_log", "")
    if session_log:
        try:
            r = subprocess.run(["lsof"], capture_output=True, text=True)
            pids = []
            for line in r.stdout.splitlines():
                if session_log in line:
                    parts = line.split()
                    if len(parts) > 1 and parts[1].isdigit() and int(parts[1]) not in self_pids:
                        pids.append(parts[1])
            if pids:
