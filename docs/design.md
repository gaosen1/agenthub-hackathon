...
| 版本 | v0.2（融合张子剑 / 高森两版设计） |
| 团队 | Team 055（杭州赛区）：张子剑（俊良）、高森（松间客） |
| 文档状态 | Draft |
| 最后更新 | 2026-08 |
...
包格式融合两版设计：**git bundle 负责代码增量与合并**（高森版），**qwen-home 目录负责 session 可 resume**（张版）。
...
Hub 持有 OSS 凭证（复用 RAM 角色 `agenthubosshandoffrole`），CLI 与 Sandbox 均不持长期凭证，只拿限时（30min）、限 key 的签名 URL。
...
#### F-12 Web 任务面板（P0，UI 以 dev/gaosen 原型 docs/prototype.html 为准）
...
复用云资源：ACK 集群 `qwen-code-demo`（杭州，ACS 算力，新建 ns `agenthub`）、OSS bucket `qwen-agent-hub-handoffs-1676569025647691`、RAM 角色 `agenthubosshandoffrole`；需新建 ACR 个人版镜像仓库。
