...
describe('db 迁移', () => {
  ...
  it('旧库（有表、无版本号）升级后数据不丢且版本到位', () => {
    const db = legacyDb();
    db.prepare("INSERT INTO users (id, username, password_hash, created_at) VALUES (1,'gaosen','h','t0')").run();
    db.prepare(
    ...
    expect(version(db)).toBe(MIGRATIONS.length);
    expect(db.prepare('SELECT username FROM users WHERE id=1').get()).toEqual({ username: 'gaosen' });
